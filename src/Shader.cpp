...
bool Shader::create(const std::string vertexShaderFilename,
    ...
    if (!isExtensionOK) {
       ...
       cerr << "GLAD version = 2.0: " << (GLAD_GL_VERSION_2_0!=0) << std::endl;
       cerr << "ARB Vertex shader: " << (GLAD_GL_ARB_vertex_shader!=0) << std::endl;
       cerr << "ARB Fragment shader: " << (GLAD_GL_ARB_fragment_shader!=0) << std::endl;
       assert(false);
       return false;
    }

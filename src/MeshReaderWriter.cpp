...
void MeshReaderWriter::loadOBJFile(const string &filename, Mesh& meshToCreate) {
    ...
    if (!normals.empty()) meshNormals = vector<glm::vec3>(vertices.size(), glm::vec3(0, 0, 0));
    vector<glm::vec2> meshUVs;
    if (!uvs.empty()) meshUVs = vector<glm::vec2>(vertices.size(), glm::vec2(0, 0));
    ...
    for(size_t i = 0, j = 0; i < verticesIndices.size(); j++) {
        ...
        if (!isQuad[j]) {
            ...
            glm::vec3 v4 = vertices[i4];

            glm::vec3 v13 = v3-v1;
            glm::vec3 v12 = v2-v1;
            glm::vec3 v14 = v4-v1;

            float a1 = glm::dot(v13, v12);
            float a2 = glm::dot(v13, v14);
...
void MeshReaderWriter::writeOBJFile(const std::string& filename, const Mesh& meshToWrite) {
    ...
    // Geth the mesh data
    const std::vector<glm::vec3>& vertices = meshToWrite.getVertices();
    const std::vector<glm::vec3>& normals = meshToWrite.getNormals();
    const std::vector<glm::vec2>& uvs = meshToWrite.getUVs();
